...
class Transmission_Model:
    ...
    def init_distance(self):
        np.random.seed(1)
        self.user_distance, self.user_angles = self.lower_limit_distance + (self.upper_limit_distance - self.lower_limit_distance) * np.random.rand(self.user_number, 1), 2 * np.pi * np.random.rand(self.user_number)
        np.random.seed()
    ...
    def init_user_data_rate(self):
        ...
        A common digital modulation technique is 8PSK (Phase Shift Keying with 8 phases).
        Considering the 8PSK technique, which modulates 8 bits for each symbol, the bit rate per symbol is equivalent to 1 byte.
    ...
    def init_total_energy(self):
        self.total_energy = self.energy_coeff * self.cpu_cycles * (
                self.cpu_freq ** 2) * self.data_size_model + self.user_power * self.user_delay
